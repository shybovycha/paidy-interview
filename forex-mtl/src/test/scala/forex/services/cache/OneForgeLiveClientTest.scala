...
import forex.services.rates.oneforge.{LiveInterpreter, QuoteDTO}
...
class OneForgeLiveClientTest extends FunSuite {
  ...
  test("#fetchPossiblePairs returns all currency pairs available") {
    val client = new LiveInterpreter[IO](config)
  ...
  test("#convertRateUri returns CanNotParseConvertUri when unable to parse URI") {
    ...
    val client = new LiveInterpreter[IO](brokenConfig)
  ...
  test("#handleHttpError converts timeout exception to error") {
    ...
    LiveInterpreter.handleHttpError(exception) should be (NetworkFailure(exception))
  }
  ...
  test("#handleHttpError converts invalid message body exception to error") {
    ...
    LiveInterpreter.handleHttpError(exception) should be (BadResponseFailure(exception))
  }
  ...
  test("#handleHttpError converts any other exception to error") {
    ...
    LiveInterpreter.handleHttpError(exception) should be (UnknownFailure(exception))
  }
